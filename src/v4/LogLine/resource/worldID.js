import { getServerRegion } from "../../xivapi/api";

export const worldIDtoName = (worldId) => {

    // Convert worldId to a string if it's a number, as the keys in Worlds are strings
    const key = String(worldId);
    if (Worlds[key]) {
        return { name: Worlds[key].name, dc: Worlds[key].dataCenter.name, region: getServerRegion(Worlds[key].dataCenter.name) };
    }
    return { name: "", dc: "", region: "" }; // Return undefined if the world ID is not found
};

//https://github.com/OverlayPlugin/cactbot/blob/main/resources/world_id.ts
const Worlds = {
    '21': {
        'dataCenter': {
            'id': 9,
